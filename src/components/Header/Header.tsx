import { useContext } from "react";
import { StyledHeader } from "./styles";
import { CartContext } from "../../providers/CartContext";
import { ProductsContext } from "../../providers/ProductsContext";
import { UserContext } from "../../providers/UserContext";

export const Header = () => {
  const { setCartModal, setWishListModal } = useContext(CartContext);
  const { redirectToLogin } = useContext(ProductsContext);
  const {userLogout} = useContext(UserContext)

  const token = localStorage.getItem("@epicStyle:token") || "";

  return (
    <StyledHeader>
      <div className="header__totalContent">
        <div className="header__logo">
          <h1>EPIC STYLE</h1>
        </div>
        {!token ? (
          <div className="header__menu">
            <div className="header__menu__left">
              <button className="header__button__userProfile">
                <img
                  src="./src/assets/header/png/user.png"
                  alt="Perfil do usuário"
                />
              </button>
              <button
                onClick={() => setCartModal(true)}
                className="header__button__cart"
              >
                <img
                  src="./src/assets/header/png/cart.png"
                  alt="Carrinho de compras"
                />
              </button>
              <button
                onClick={() => setWishListModal(true)}
                className="header__button__wishes"
              >
                <img
                  src="./src/assets/header/png/wishes.png"
                  alt="Lista de desejos"
                />
              </button>
            </div>
            <div className="header__menu__right">
              <button className="header__button__login" onClick={() => redirectToLogin()}>
                Entrar
                <img
                  className="header__button__signIn"
                  src="./src/assets/header/png/enter.png"
                  alt="Entrar"
                />
              </button>
            </div>
          </div>
        ) : (
          <div className="header__menu">
            <div className="header__menu__left">
              <button
                onClick={() => setCartModal(true)}
                className="header__button__cart"
              >
                <img src="./src/assets/header/png/cart.png" alt="Carrinho de compras" />
              </button>
              <button
                onClick={() => setWishListModal(true)}
                className="header__button__wishes"
              >
                <img src="./src/assets/header/png/wishes.png" alt="Lista de desejos" />
              </button>
            </div>
            <div className="header__menu__right">
              <button className="header__button__userProfile">
                <img src="./src/assets/header/png/user.png" alt="Perfil do usuário" />
              </button>
              <button className="header__button__leave" onClick={() => userLogout()}>
                <img src="./src/assets/header/png/leave.png" alt="Entrar" />
              </button>
            </div>
          </div>
        )}
        </div>
    </StyledHeader>
  );
};
